...
author: "A.Otiniano (co-author: J.Andrade)"
institute: "AoZ Geoanalytics And Multidisciplinary Sensing, Universal Accessibility and Machine Learning Group, National University of Engineering"
...
# Un momento voy a tomar mi muestra de agua! (preguntas?)
...
#Realizaremos los siguientes ejemplos (seguir la correcta estructura):
set.seed(123)
x <- rnorm(10, mean = 18, sd = 5.8)
# Calcular cuantiles
Q1 <- quantile(x, 0.25)
Q3 <- quantile(x, 0.75)
# Categorize the data using ifelse
categories <- ifelse(x < Q1, "bajo",
                     ifelse(x >= Q1 & x <= Q3, "medio", "alto"))
# Combinar data para visualizacion en data frame
result <- data.frame(Value = x, Category = categories)
print(result)
```
...
set.seed(123)
# Generar la data
data_model <- rnorm(n = 10, mean = 5.8, sd = 2.4)
# Calcular cuantiles
Q1 <- quantile(data_model, 0.25)
Q3 <- quantile(data_model, 0.75)
# Vector inizializado para guardar categorias
categories <- character(length(data_model))
# Categorizar la data basado en cuantiles
for (i in seq_along(data_model)) {
  if (data_model[i] < Q1) {
    categories[i] <- "bajo"
  } else if (data_model[i] >= Q1 & data_model[i] <= Q3) {
    categories[i] <- "medio"
  } else {
    categories[i] <- "alto"
  }
}
# Combinar la data y categorias en un datagrame
result <- data.frame(Value = data_model, Category = categories)
